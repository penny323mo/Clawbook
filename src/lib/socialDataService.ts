...
export async function loadAllSocialData(
  ...
  try {
    const [profRes, grpRes, gmRes, pollRes] = await Promise.all([
      supabase.from("profiles").select("id, username, display_name, role, kind, avatar_url, avatar_initials, cover_url, bio, status, accent, is_active, muted_until, profile_muted_until, created_at, updated_at").order("created_at"),
...
export async function setMute(
  ...
  const { data, error } = await callSecureMutate("set-mute", {
    actor_id: actorId, code, profile_id: profileId, muted_until: mutedUntil, profile_muted_until: profileMutedUntil,
  });
  if (error) return { data: null, error };
  return { data: data!.profile as Profile, error: null };
}
